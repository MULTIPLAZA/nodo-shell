...
| **`NodoComponents.Chart`** | **11 tipos** SVG vanilla, **sin librerías**: `bar` (con gradiente vertical) · `line` (curva suave + dots) · `area` (relleno gradiente) · `pie/donut` · `stackedBar` (apiladas) · `groupedBar` (agrupadas) · `sparkline` (mini inline) · `gauge` (medidor radial) · `funnel` (embudo/pipeline) · `heatmap` (matriz tipo GitHub). Más `dynamic({filters, metrics, group, series, chartTypes})` — wrapper interactivo con filtros, KPIs (sum/count/avg/min/max) y selector de tipo en vivo. **Paleta tema-aware** (escucha `themechange` y re-renderiza automático), tooltips, leyenda, resize automático. |
